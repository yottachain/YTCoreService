...
func StartPreAllocNode() {
	...
	DNList = &NodeList{list: make(map[int32]*NodeStat), updateTime: 0, resetSign: new(int32), r:rand.New(rand.NewSource(time.Now().UnixNano()))}
	*DNList.resetSign = 0
	go func() {
		for {
	...
	if ok {
		if resp.Preallocnode != nil && len(resp.Preallocnode) > 0 {
			...
			if nlen > 0 {
				...
				DNList.SetwIds(uint(env.OptionMiners), uint(env.WeightDivsor))
				logrus.Infof("[PreAllocNode]Return %d nodes,Excludes %d nodes, current %d nodes.\n",
					nlen, len(req.Excludes), len(DNList.list))
				return nil
			}
		}
