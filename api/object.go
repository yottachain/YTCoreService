...
func (accessor *ObjectAccessor) CreateObject(bucketname, filename string, VNU primitive.ObjectID, meta []byte) *pkt.ErrorMessage {
	req := &pkt.UploadFileReqV2{
		UserId:     &accessor.UClient.UserId,
		SignData:   &accessor.UClient.SignKey.Sign,
		KeyNumber:  &accessor.UClient.SignKey.KeyNumber,
...
func (accessor *ObjectAccessor) CopyObject(srcbuck, srckey, destbuck, destkey string) (*FileItem, *pkt.ErrorMessage) {
	...
	if errmsg != nil {
		logrus.Errorf("[CopyObject][%d]%s/%s-->%s/%s ERR:%s\n", accessor.UClient.UserId, srcbuck, srckey, destbuck, destkey, pkt.ToError(errmsg))
		return nil, errmsg
	}
...
func (accessor *ObjectAccessor) ListObject(buck, fileName, prefix string, wversion bool, nVerid primitive.ObjectID, limit uint32) ([]*FileItem, *pkt.ErrorMessage) {
	...
	if errmsg != nil {
		logrus.Errorf("[ListObject][%d]%s/%s/%s/%s ERR:%s\n", accessor.UClient.UserId, buck, fileName, prefix, nVerid.Hex(), pkt.ToError(errmsg))
		return nil, errmsg
	}
	respv1, OK1 := resp.(*pkt.ListObjectResp)
	if OK1 {
		ret, errmsg := accessor.GetListRespV1(respv1)
		if errmsg != nil {
			logrus.Errorf("[ListObject][%d]%s/%s/%s/%s ERR:%s\n", accessor.UClient.UserId, buck, fileName, prefix, nVerid.Hex(), pkt.ToError(errmsg))
			return nil, errmsg
		} else {
			logrus.Infof("[ListObject][%d]%s/%s/%s/%s OK,count:%d\n", accessor.UClient.UserId, buck, fileName, prefix, nVerid.Hex(), len(ret))
			return ret, nil
		}
	}
	respv2, OK2 := resp.(*pkt.ListObjectRespV2)
	if OK2 {
		ret, errmsg := accessor.GetListRespV2(respv2)
		if errmsg != nil {
			logrus.Errorf("[ListObject][%d]%s/%s/%s/%s ERR:%s\n", accessor.UClient.UserId, buck, fileName, prefix, nVerid.Hex(), pkt.ToError(errmsg))
			return nil, errmsg
		} else {
	...
	logrus.Errorf("[ListObject][%d]%s/%s/%s/%s return err msg type.\n", accessor.UClient.UserId, buck, fileName, prefix, nVerid.Hex())
	return nil, pkt.NewErrorMsg(pkt.SERVER_ERROR, "Return err msg type")
}

func (accessor *ObjectAccessor) GetListRespV1(resp *pkt.ListObjectResp) ([]*FileItem, *pkt.ErrorMessage) {
	items := []*FileItem{}
...
func (accessor *ObjectAccessor) GetListRespV2(resp *pkt.ListObjectRespV2) ([]*FileItem, *pkt.ErrorMessage) {
	...
	return accessor.GetListRespV1(msg)
}

func (accessor *ObjectAccessor) DeleteObject(buck, fileName string, Verid primitive.ObjectID) *pkt.ErrorMessage {
	...
	_, errmsg := net.RequestSN(req)
	if errmsg != nil {
		logrus.Errorf("[DeleteObject][%d]%s/%s ERR:%s\n", accessor.UClient.UserId, buck, fileName, pkt.ToError(errmsg))
		return errmsg
	} else {
		logrus.Infof("[DeleteObject][%d]%s/%s OK.\n", accessor.UClient.UserId, buck, fileName)
		return nil
	}
}

func (accessor *ObjectAccessor) ObjectExist(buck, fileName string) (bool, *pkt.ErrorMessage) {
	id, err := accessor.GetObjectId(buck, fileName)
	if err != nil {
		return false, err
	} else {
...
func (accessor *ObjectAccessor) GetObjectId(buck, fileName string) (primitive.ObjectID, *pkt.ErrorMessage) {
	id, err := accessor.GetObject(buck, fileName)
	if err != nil {
...
func (accessor *ObjectAccessor) GetObject(buck, fileName string) (*FileItem, *pkt.ErrorMessage) {
	key := fmt.Sprintf("[%d]%s/%s", accessor.UClient.UserId, buck, fileName)
