...
var LRCBugTime = func() int64 {
	t, _ := time.Parse("2006-01-02", "2022-03-01")
	return t.Unix()
}()
...
func (down DownloadBlock) loadLRCShard(resp *pkt.DownloadBlockInitResp, resp2 *pkt.DownloadBlockInitResp2) (*codec.EncryptedBlock, *pkt.ErrorMessage) {
	ignid := 0
	if LRCBugTime > down.Ref.VBI>>32 {
		if down.Ref.RealSize >= 1024*16 && down.Ref.RealSize < 1024*48 {
			if resp != nil {
	...
	dns := NewDownLoad(fmt.Sprintf("[%d][%d]", down.Ref.Id, down.Ref.VBI), 0, ignid)
...
func (down DownloadBlock) loadCopyShard(resp *pkt.DownloadBlockInitResp) (*codec.EncryptedBlock, *pkt.ErrorMessage) {
	vhf := resp.Vhfs.VHF[0]
	var b []byte
	dns := NewDownLoad(fmt.Sprintf("[%d][%d]", down.Ref.Id, down.Ref.VBI), len(resp.Nlist.Ns), 0)
	for _, n := range resp.Nlist.Ns {
		if n != nil {
