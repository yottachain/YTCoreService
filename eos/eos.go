...
const AddUsedSpaceENABLE = false

func AddUsedSpace(username string, length uint64) error {
	if !env.BP_ENABLE || !AddUsedSpaceENABLE {
		return nil
	}
...
func RequestWRetry(actname string, obj interface{}, retrytimes int) (*eos.PushTransactionFullResp, error) {
	...
	for {
		URI := GetEOSURI()
		res, err := RequestEos(actname, obj, URI)
		if err != nil {
...
func RequestEos(actname string, obj interface{}, URI *EOSURI) (*eos.PushTransactionFullResp, error) {
	api, err := URI.NewApi()
	if err != nil {
