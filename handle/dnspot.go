...
func ExecSendSpotCheck() {
	...
	if ischeck {
		ctx2, cancel2 := context.WithTimeout(context.Background(), time.Second*time.Duration(env.P2P_ReadTimeout))
		defer cancel2()
		...
		req := &pkt.SpotCheckTaskList{TaskId: list.TaskID.Hex(), Snid: 0}
