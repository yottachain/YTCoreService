...
func (h *UploadBlockInitHandler) Handle() proto.Message {
	...
	if env.Version != "" {
		if h.m.Version == nil || *h.m.Version == "" || bytes.Compare([]byte(*h.m.Version), []byte(env.Version)) < 0 {
			v := "NULL"
			if h.m.Version != nil {
				v = *h.m.Version
			}
...
func (h *UploadBlockDBHandler) Handle() proto.Message {
	...
	ref := &pkt.Refer{VBI: vbi, Dup: 0, ShdCount: 0, OriginalSize: int64(*h.m.OriginalSize),
...
func (h *UploadBlockDupHandler) Handle() proto.Message {
	...
	ref := &pkt.Refer{VBI: meta.VBI, Dup: 1, ShdCount: vnf, OriginalSize: int64(*h.m.OriginalSize),
...
func (h *UploadBlockEndHandler) Handle() proto.Message {
	...
	logrus.Infof("[UploadBLK]/%s/%d OK,take times %d ms\n", h.vnu.Hex(), *h.m.Id, time.Since(startTime).Milliseconds())
	return &pkt.UploadBlockEndResp{VBI: &vbi}
}
